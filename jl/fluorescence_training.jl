...
function main_opt()
  info(" Starting training function...", prefix=string(now(), " INFO: "))
  ...
  for i = 1:num_methods
    ...
    info(" using $opt_method...", prefix=string(now(), " INFO: "))
    tic()
    try 
  ...
  info(" optimisation results written to $filename...", prefix=string(now(), " INFO: "))
